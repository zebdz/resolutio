...
export function SidebarDivider({
  ...
  return (
    <hr
      {...props}
      className={clsx(className, 'my-4 border-t border-white/10 lg:-mx-4')}
    />
  );
}
...
export function SidebarHeading({
  ...
  return (
    <h3
      ...
      className={clsx(
        className,
        'mb-1 px-2 text-xs/6 font-medium text-white/60'
      )}
    />
  );
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  const classes = clsx(
    ...
    // Dark mode
    'dark:text-white dark:*:data-[slot=icon]:fill-white/70',
    ...
    <span className={clsx(className, 'relative')}>
      {current && (
        <motion.span
          ...
          className="absolute inset-y-2 -left-4 w-0.5 rounded-full bg-brand-yellow"
        />
      )}
