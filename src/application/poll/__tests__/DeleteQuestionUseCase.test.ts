...
describe('DeleteQuestionUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new DeleteQuestionUseCase(pollRepository);
  });
  ...
  it('should delete (archive) question when user is poll creator and poll is editable', async () => {
    ...
    // Verify question was archived
    const deletedQuestionResult =
      await pollRepository.getQuestionById('question-1');
    expect(deletedQuestionResult.success).toBe(true);
    const deletedQuestion = deletedQuestionResult.value!;
    expect(deletedQuestion.isArchived()).toBe(true);
  });
