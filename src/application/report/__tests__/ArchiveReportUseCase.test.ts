...
describe('ArchiveReportUseCase', () => {
  ...
  let orgs: FakeOrgs;
  let users: FakeUsers;
  let uc: ArchiveReportUseCase;

  beforeEach(() => {
    reports = new FakeReports();
    orgs = new FakeOrgs();
    users = new FakeUsers();
    uc = new ArchiveReportUseCase(reports as any, orgs as any, users as any);
    reports.store.set('r1', makeReport());
  });

  it('author archives the report', async () => {
    const r = await uc.execute({ reportId: 'r1', callerId: 'author-1' });
    expect(r.success).toBe(true);
    expect(reports.updated[0].isArchived()).toBe(true);
  });

  it('admin archives the report', async () => {
  ...
  it('superadmin archives the report', async () => {
  ...
  it('double-archive rejected (domain REPORT_ALREADY_ARCHIVED)', async () => {
    reports.store.set('r1', makeReport({ archivedAt: new Date() }));
    const r = await uc.execute({ reportId: 'r1', callerId: 'author-1' });
    expect(r.success).toBe(false);

    if (!r.success) {
      expect(r.error).toBe(ReportDomainCodes.REPORT_ALREADY_ARCHIVED);
    }
  });
