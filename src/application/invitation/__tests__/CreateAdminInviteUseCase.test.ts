...
import { CreateAdminInviteUseCase } from '../CreateAdminInviteUseCase';
...
describe('CreateAdminInviteUseCase', () => {
  let useCase: CreateAdminInviteUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new CreateAdminInviteUseCase({
  ...
  it('should fail when invitee is already admin', async () => {
    orgRepo.addOrganization(makeOrg('org-1'));
    orgRepo.setAdmin('org-1', 'admin-1');
    ...
    if (!result.success) {
      expect(result.error).toBe(InvitationErrors.INVITEE_ALREADY_ADMIN);
    }
  });
  ...
  it('should succeed and create invitation', async () => {
    orgRepo.addOrganization(makeOrg('org-1'));
    orgRepo.setAdmin('org-1', 'admin-1');
    userRepo.addUser(makeUser('user-1'));
    ...
    if (result.success) {
      expect(result.value.type).toBe('admin_invite');
  ...
  it('should allow re-invite after decline', async () => {
    ...
    userRepo.addUser(makeUser('user-1'));
    userRepo.addUser(makeUser('admin-1'));
    // Add a declined invite (should not block new invite)
    invitationRepo.addInvitation(
      Invitation.reconstitute({
        id: 'declined-inv',
        organizationId: 'org-1',
