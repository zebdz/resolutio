...
import { ReactivateJoinTokenUseCase } from '../ReactivateJoinTokenUseCase';
...
describe('ReactivateJoinTokenUseCase', () => {
  let useCase: ReactivateJoinTokenUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new ReactivateJoinTokenUseCase({
  ...
  it('succeeds when admin reactivates expired token', async () => {
    ...
    expect(tokenRepo.updatedToken!.expiredAt).toBeNull();
  });
  ...
  it('fails when token not expired', async () => {
