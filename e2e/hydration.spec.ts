...
test.describe('Hydration', () => {
  test('should not have hydration errors on register page', async ({
    page,
  }) => {
    ...
    expect(
      hydrationErrors,
      `Found hydration errors: ${hydrationErrors.join('\n')}`
    ).toHaveLength(0);
    expect(
      pageErrors,
      `Found page errors: ${pageErrors.map((e) => e.message).join('\n')}`
    ).toHaveLength(0);
  });
  ...
  test('html lang attribute should be consistent (ru)', async ({
    page,
    context,
  }) => {
